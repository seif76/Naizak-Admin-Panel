  ...
  const fetchOrders = async () => {
    ...
    try {
      const res = await axios.get(`${BACKEND_URL}/api/admin/orders`, {
        params: { page, limit: 10, status: 'confirmed' },
      });
    ...
    <div className="p-6">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-gray-600">Orders ready for shipping</p>
        </div>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
        <div className="flex items-center gap-2 text-blue-700">
          <FaCheck />
          <span className="text-lg font-semibold">{orders.length} Confirmed Orders</span>
        </div>
      </div>
        ...
        <table className="w-full text-left min-w-[900px]">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={9} className="text-center p-5">No confirmed orders found</td></tr>
            ) : (
      ...
      <div className="mt-4 flex justify-center gap-2">
        {[...Array(totalPages).keys()].map((num) => (
          <button
            key={num + 1}
            className={`px-3 py-1 rounded border ${page === num + 1 ? 'bg-blue-500 text-white' : 'bg-white text-gray-800'}`}

    ...
    <div className="p-6">
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
        <div className="flex items-center gap-2 text-yellow-700">
          ...
          <span className="text-lg font-semibold">{orders.length} Pending Orders</span>
        </div>
      </div>
        ...
        <table className="w-full text-left min-w-[900px]">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={9} className="text-center p-5">No pending orders found</td></tr>
            ) : (
      ...
      <div className="mt-4 flex justify-center gap-2">
        {[...Array(totalPages).keys()].map((num) => (
          <button
            key={num + 1}
            className={`px-3 py-1 rounded border ${page === num + 1 ? 'bg-yellow-500 text-white' : 'bg-white text-gray-800'}`}
